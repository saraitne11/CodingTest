...
def dfs(program, n, m):
    ...
    def search(current, visited):
        ...
        if code == AT:              # '@'
            ...
            new_i, new_j = update_location(i, j, new_di, new_dj, n, m)
            new_current = [new_i, new_j, v, new_di, new_dj]
            search(new_current, visited + new_current)
            ...
            raise ValueError('%s %s' % (code, [i, j, v, di, dj]))

    start_i, start_j = (0, 0)
    start_v = 0
    start_di, start_dj = MOVES[RIGHT]
